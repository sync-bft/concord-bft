...
void ReplicaImp::onMessage<ClientRequestMsg>(ClientRequestMsg *m) {
  ...
  if (readOnly) { //guess: requests that don't need execution do not need to achieve consensus or futher the storage?
    executeReadOnlyRequest(span, m);
    delete m;
    return;
  }

  //ignore the method if not in the matching view
  ...
  if (seqNumberOfLastReply < reqSeqNum) { // another sanity check: avoid msgs that are redundant 
    if (isCurrentPrimary()) {
      ...
      if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum)) {
        LOG_DEBUG(CNSUS,
                  "Pushing to primary queue, request [" << reqSeqNum << "], client [" << clientId
                                                        << "], senderId=" << senderId);
        requestsQueueOfPrimary.push(m); // added to the processing line
        primaryCombinedReqSize += m->size();
        tryToSendPrePrepareMsg(true); // initialize the slow path pipeline
        return;
      } else {
        LOG_INFO(GL,
      ...
      if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum)) {
        ...
        send(m, currentPrimary()); // send the msg to the primary but why it will view-change?
...
void ReplicaImp::tryToSendPrePrepareMsg(bool batchingLogic) {
  ...
  // the last used seqNum exceeds the seqNum that the primary replica could execute (concurrency means in pa)
  ...
  //queue: what exactly does the queue store?
  ...
  if (requestsInQueue == 0) return; //no requests to primary from the client
  ...
  //after the update of the client request queue, the next request is at the front position
  ...
  if (pp->numberOfRequests() == 0) { // no client requests to be processed
...
void ReplicaImp::onInternalMsg(FullCommitProofMsg *msg) { // on the upper of onMessage 
  if (isCollectingState() || (!currentViewIsActive()) || (curView != msg->viewNumber()) ||
      (!mainLog->insideActiveWindow(msg->seqNumber()))) { //(not collector(?), view is active, msg view and replica view match, seqNum is within the window)
    delete msg;
    return;
  }
  onMessage(msg);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// below are infrastructure of the protocol
